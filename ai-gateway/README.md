...
**Это отдельный, самостоятельный сервис.** Он не подключён к
Go-бэкенду `nexd` и не участвует в сборке/тестах основного проекта —
специально вынесен в свою папку с собственными зависимостями, чтобы
ничего не мешало Go-части. Соответствие с реальным (Go) AI-планом NEX —
только концептуальное, для сравнения; см. `docs/ai/README.md`, §5.
...
Клиент (curl / браузер / фронтенд)
        ...
        │  заголовок X-Tenant-Id (опционально, иначе тенант "default")
        ▼
┌────────────────────┐
...
Оба эндпоинта принимают опциональный заголовок `X-Tenant-Id`:
...
Это учебный сервис, а не прод-система, поэтому в нём **нет**:

- RBAC/авторизации (кто вообще имеет право звать AI) и аудита (кто и
  что спросил) — в реальном NEX это дал бы command bus (см.
  `docs/architecture-go.md`, §4, §6) и то, что `docs/ai/README.md`
  называет «AI как актор шины команд». Заголовок `X-Tenant-Id`
  сейчас — это самоидентификация клиента, а не проверенная авторизация:
  ничто не мешает клиенту представиться чужим тенантом;
