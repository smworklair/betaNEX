...
|---|---|---|---|
| **Сегодня** | Реально работает | `ai-gateway/` (отдельный сервис) + фронтенд как клиент | Python (FastAPI) — шлюз; TypeScript/React — клиент, вызывающий только шлюз |
| **План (roadmap)** | Спроектировано, не реализовано | Go-бэкенд | `internal/platform/llm` (пакет пока не существует) |
...
из браузера, а ключи лежали в `localStorage`. Это было исправлено —
подробности в разделе 1. Целевая архитектура на Go (`internal/platform/llm`,
раздел 2) при этом остаётся планом: `ai-gateway` — рабочий, но
промежуточный шаг на другом языке, не финальная архитектура NEX.

## 1. Как это устроено сегодня

Все вызовы LLM идут через отдельный сервис **`ai-gateway/`** (Python,
FastAPI, см. [`ai-gateway/README.md`](../../ai-gateway/README.md)) —
фронтенд больше не обращается к внешним AI API напрямую и не хранит
никаких ключей в браузере:

```ts
// web/src/llm.ts
// Конфигурация — через VITE_AI_GATEWAY_URL (см. .env.example):
//   • пусто / не задано → демо-режим: ИИ выключен, все точки входа сами
...
sequenceDiagram
    ...
    R->>GW: POST /api/v1/ai/ask {message, history, context, provider?}
    GW->>GW: rate-limit → budget.check(tenant) → context_registry → system-промпт
    GW->>Prov: запрос провайдеру (ключ — из env самого ai-gateway)
    Prov-->>GW: текст ответа + usage
    GW->>GW: budget.record(usage)
    GW-->>R: {text, provider, model, usage}
    R-->>U: показать ответ
    Note over R,GW: Go-бэкенд (nexd) в этом потоке по-прежнему НЕ участвует —\nai-gateway работает как самостоятельный процесс на своём порту (8090)
```

`nexd` (Go) по-прежнему не участвует ни в одном AI-запросе — это
единственное, что не изменилось по сравнению с более ранней версией
этого документа. Изменилось то, ЧТО именно вызывает браузер: не внешний
провайдер напрямую, а собственный сервис `ai-gateway`.
...
уходит в тот же `llm.ts`, который теперь стучится в `ai-gateway`, а не
напрямую во внешний провайдер — но `nexd` (Go) по-прежнему не участвует
в этом пути. Это решение важно для раздела 3 (безопасность) — оно и
есть первая линия защиты от prompt injection на Go-сервере: он такие
тексты просто не интерпретирует.

### 1.6 Конфигурация сегодня

- **`ai-gateway/.env.example`** — все ключи провайдеров, таймауты,
  бюджеты, rate-limit; см. `ai-gateway/README.md`.
- **`web/.env.example`** — `VITE_AI_GATEWAY_URL`: пусто = демо-режим без
  сети, URL = реальные вызовы к шлюзу. Ключей провайдеров здесь больше
  нет вообще.
- **Корневой `.env.example`** (Go-бэкенд `nexd`) — AI-переменных по-прежнему
  нет: `nexd` не участвует в AI-потоке (см. 1.1).
- **`docker-compose.yaml` / `deploy/compose.prod.yaml`** — `ai-gateway`
  включён в docker-compose для локальной разработки (см. `docker-compose.yaml`
  в корне); прод-стек — см. `deploy/compose.prod.yaml`.
