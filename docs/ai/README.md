...
**Fallback-цепочка.** Если клиент НЕ указал `provider` явно, сервис
пробует провайдеров по очереди из `PROVIDER_FALLBACK_CHAIN` (по
умолчанию `deepseek → kimi → gemini`) и переключается на следующего при
отказе (таймаут, 5xx, auth-ошибка, пустой ответ) — см.
`app/services/ai_service.py:_resolve_chain` и подробности в
[`ai-gateway/README.md`, разделе «Fallback-цепочка между
провайдерами»](../../ai-gateway/README.md#fallback-цепочка-между-провайдерами).
Явный `provider` в запросе фолбэк не включает — это то же правило, что
и для ru-restricted маршрута ниже (§3): выбор провайдера клиентом
уважается буквально. Для `/stream` переключение возможно только до
первого отданного клиенту чанка.

**Цены за токен.** `*_PRICE_INPUT_PER_1K_USD`/`*_PRICE_OUTPUT_PER_1K_USD`
в `.env` заполнены публичными прайс-листами провайдеров по состоянию на
июль 2026 — таблица с источниками в [`ai-gateway/README.md`, разделе
«Цены за токен»](../../ai-gateway/README.md#цены-за-токен). Это ОРИЕНТИР
для учебного бюджетирования (лимиты `*_cost_usd` теперь реально
срабатывают), не прайс-лист для боевого биллинга.

### 1.4 Откуда AI вызывается в UI

Точки входа вызывают `llmAsk()` (обычный запрос, ждать полный ответ),
`llmAskStream()` (потоковый, печатает по мере генерации) или `llmPlan()`
из `llm.ts`, который стучится в `ai-gateway` через `nexd`, а не во
внешний провайдер напрямую:

| Файл | Что это | Стриминг | Поведение при отсутствии шлюза/ошибке |
|---|---|---|---|
| `web/src/ai.tsx` | Оверлей «AI везде»: `ProactiveStrip`, `InlinePanel` (чат-панель на странице, история — своя на раздел/студента), `SelectionPopover` (меню по выделению текста) | Нет — `llmAsk()` | Падает обратно на локальный детерминированный мок `nexReply()` (`web/src/nexbrain.ts`) |
| `web/src/pages/Chat.tsx` | Полноэкранная страница чата, история последних 8 сообщений, явный `system: ORG_CONTEXT` | Да — `llmAskStream()`, ответ печатается по мере генерации | Пустой стрим → `llmAsk()` → `nexReply()` |
| `web/src/pages/aibox.tsx` | Переиспользуемый мини-чат `<AiBox>` для разделов — передаёт `context` (page/facts), хранит свою историю на раздел | Да — то же самое | Тот же трёхуровневый fallback |
| `web/src/pages/agents.tsx` | Экран «Агенты»: 7 вымышленных именованных агентов (`web/src/agents.ts`) с уровнями автономии | Нет — `llmPlan()` внутри вызывает `llmAsk()` | Реальный вызов `llmPlan()`, если шлюз сконфигурирован, иначе заготовленный `planFor()`. **Все «действия» агентов — таймеры и локальный React-стейт, реального выполнения на бэкенде нет** |
...
**Как устроен стриминг во фронтенде.** `llmAskStream()` шлёт
`POST /api/v1/ai/stream` через `fetch` и разбирает `ReadableStream`
вручную (не `EventSource` — тому нечем отправить POST-тело и
`credentials`), вызывая `onDelta(text)` на каждый чанк `event: delta`.
Если поток оборвался, уже показав пользователю часть текста (провайдер
упал в середине генерации — фолбэк на другого провайдера в этот момент
уже невозможен, см. §1.3 выше), частичный текст всё равно считается
результатом — обрывать на середине хуже, чем показать то, что успело
прийти. `LlmStreamError` бросается, только если не пришло вообще
ничего — тогда `AiBox`/`Chat.tsx` откатываются на `llmAsk()`, а при его
отказе — на локальный демо-мок, как и раньше. Per-раздел история
(`sessionStorage`, см. §1.2) сохраняется уже ПОСЛЕ того, как ответ
полностью получен — сам стриминг никак её не меняет.
...
- **Бюджеты, rate-limit, кэш и fallback — теперь все пять звеньев
  декораторной цепочки из §2.4 есть и в Python-прототипе.**
  `Router → Budget → Cache → RateLimit → Fallback` в Go-плане
  соответствует тому, что в `ai-gateway` реализовано как: Router —
  выбор провайдера/цепочки в `AIService._resolve_chain`; Budget —
  `budget_service.py`; Cache — `core/response_cache.py` (TTL, изоляция
  по тенантам, кэш-хит не тратит бюджет); RateLimit — `core/
  ratelimit.py`; Fallback — `PROVIDER_FALLBACK_CHAIN` в `AIService.ask`/
  `ask_stream` (переключение на следующего провайдера при отказе, если
  клиент сам не закрепил конкретный `provider`). Различие с Go-планом —
  не в наборе звеньев, а в том, что здесь это не отдельные
  Client-декораторы, оборачивающие друг друга, а условные ветки внутри
  одного `AIService` — для Python-прототипа этого достаточно, но в
  Go-реализации (раздел 2.4) стоит держаться декораторной формы: она
  проще тестировать по отдельности и переставлять местами.
...
- `docs/architecture-go.md` — общая архитектура Go-бэкенда, в которую
  впишется AI-актор.
- [`beta/Nex-pilot`](../../beta/Nex-pilot) — отдельная учебная песочница
  (CLI-ассистент поверх `FrankAI`, опционально поверх `ai-gateway`), НЕ
  часть плана этого документа. Упомянута здесь только потому, что её
  `GatewayBackend` — единственный клиент `ai-gateway` в репозитории,
  который ходит в шлюз НАПРЯМУЮ, минуя `nexd` (это CLI-утилита, а не
  браузер): он подписывает запрос тем же `X-Gateway-Secret`, что и
  `nexd` (см. `internal/platform/httpapi/aiproxy.go` и `ai-gateway/app/
  deps.py:verify_gateway_secret`) — те же правила доверия, тот же
  секрет, просто без прослойки Go-прокси.
