    ...
    <div className="fade content-narrow">
      ...
      {tab === 'reg' && (
        ...
        <AiBox
          ...
          page="admissions"
          facts={[
            'Прикладная информатика: проходной ~240',
            'Информационные системы: проходной ~235',
            'Экономика и бухучёт: проходной ~220',
            'Формы обучения: очная/заочная, бюджет/контракт',
          ]}
          fallback={(q) => `По запросу «${q}»: для очной бюджетной формы нужны аттестат, паспорт, 4 фото и заявление. Проходной на ПИ ~240 баллов. Подключите ai-gateway для точных ответов по конкретному абитуриенту.`}
        />
  ...
  return (
    <div className="fade content-narrow">
      ...
      <AiBox
        ...
        facts={[
          'Поступило ~₽124K',
          'Задолженность ₽248K (8 должников, срок до 30.06)',
          '3 аномальных платежа от одного контрагента',
          'Прогноз поступлений ~₽512K/мес',
        ]}
        fallback={(q) => `По запросу «${q}»: при оплате 5 из 8 должников до 30.06 закроется ~₽155K из ₽248K долга. Точные расчёты — после подключения ai-gateway.`}
      />
