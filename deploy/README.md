...
Это ручной раннбук для одного VPS — автодеплоя НА сервер по git-тегу
здесь нет (CI лишь публикует образы в GHCR, см. ниже; сам деплой —
руками), бэкапов и полноценной наблюдаемости тоже нет (см.
`docs/roadmap.md`, вехи M9-М11; базовые логи/метрики — уже есть, см.
`docs/observability.md`).

CI (`.github/workflows/release.yml`) публикует в GHCR по git-тегу `v*`
все три образа — `nexd`, `ai-gateway` и `web` (semver+`latest`). Ниже
описан существующий способ (`ai-gateway` собирается на сервере из
исходников, фронтенд собирается локально и раздаётся Caddy как
статика) — он остаётся основным и ничего не меняется автоматически;
образы `ghcr.io/.../nex-ai-gateway` и `ghcr.io/.../nex-web` — готовая,
но пока не обязательная альтернатива сборке на сервере/вручную,
переключение на неё (замена `build:` на `image:` в
`compose.prod.yaml`) — предмет отдельного изменения раннбука.
