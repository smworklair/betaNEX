// Package cache — кэш с TTL за интерфейсом Cache (ADR-008). По
// умолчанию — in-process (Memory), без внешней инфраструктуры; когда
// nexd работает больше чем одним инстансом или нужен кэш, переживающий
// перезапуск процесса, конфигом (NEX_CACHE_BACKEND=redis) включается
// сетевая реализация (Redis, redis.go) — вызывающий код не меняется,
// он знает только про Cache.
//
